...
from visualize import km_flow_heatmap, km_flow_movie
from ..base import BaseTrainer


class PINO3DTrainer(BaseTrainer):
    ...
    def process(self, model, train_loader, valid_loader, test_loader, 
        ...
        for epoch in range(self.epochs):
            train_loss_record = self.train(model, train_loader, optimizer, criterion, scheduler, regularizer, v=kwargs['v'], dt=kwargs['dt'])
            if self.verbose:
                self.logger("Epoch {} | {} | lr: {:.6f}".format(epoch, train_loss_record, optimizer.param_groups[0]["lr"]))

            if (epoch + 1) % self.eval_freq == 0:
                valid_loss_record = self.evaluate(model, valid_loader, criterion, eval_metrics, split="valid", v=kwargs['v'], dt=kwargs['dt'])
                if self.verbose:
        ...
        test_loss_record = self.evaluate(model, test_loader, criterion, eval_metrics, split="test", v=kwargs['v'], dt=kwargs['dt'])
    ...
    def train(self, model, train_loader, optimizer, criterion, scheduler=None, regularizer=None, **kwargs):
        ...
        with tqdm(total=len(train_loader)) as bar:
            for x, y in train_loader:
                ...
                ic_loss, equation_loss = km_flow_loss(y_pred, x[:, :, :, 0, 0], v=kwargs['v'], dt=kwargs['dt'])
